...
function invokeElectronCallback(callback, returnResult = false) {
  let result;
  try {
    result = callback();
  } catch {
...
function createTrayManager({
  ...
  const handleDoubleClick = () => {
    if (!destroyed) invokeElectronCallback(showWindow);
  };
  ...
  function buildFavoriteItems(favorites, statuses) {
    ...
    function profileItem(profile) {
      ...
      return {
        ...
        click: () => invokeElectronCallback(() => launchFavorites([profile]), true),
      };
    }
    ...
    if (favorites.length > MAX_DIRECT_FAVORITES) {
      items.push({ label: '更多请在主界面操作', click: () => invokeElectronCallback(showWindow) });
    }
    return items;
  }
  ...
  function buildMenu(profiles, favorites, statuses) {
    ...
    return [
      ...
      { type: 'separator' },
      { label: '打开主界面', click: () => invokeElectronCallback(showWindow) },
      { label: '启动全部收藏', click: () => invokeElectronCallback(() => launchFavorites(getFavorites()), true) },
      { type: 'separator' },
      ...buildFavoriteItems(favorites, statuses),
      { type: 'separator' },
      { label: '退出管理器', click: () => invokeElectronCallback(requestQuit) },
    ];
  }
