  ...
  async function executeImport({ token, decisions } = {}) {
    ...
    try {
      return await profileOperations.runGlobalMutation(async () => {
        ...
        let failureCode = 'IMPORT_EXECUTION_FAILED';
        try {
          ...
          for (const plan of plannedDirectories) {
            ...
            let createdPath;
            try {
              createdPath = await createEmptyProfileDir(plan.browserType, plan.profileName);
            } catch (error) {
              if (error && error.code === 'EEXIST') {
                failureCode = 'IMPORT_DIRECTORY_CONFLICT';
              }
              throw error;
            }
            if (createdPath !== plan.profilePath) throw new Error('Profile path mismatch');
          ...
          return {
            success: false,
            code: rollbackIncomplete ? 'IMPORT_ROLLBACK_INCOMPLETE' : failureCode,
          };
        }
      });
    } catch {
