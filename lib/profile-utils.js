...
function isDuplicateProfileName(profiles, browserType, profileName, excludedId = null) {
  return profiles.some((profile) => (
    profile.id !== excludedId
    && profile.browserType === browserType
    && areProfileNamesEqual(profile.name, profileName)
  ));
}

function createCloneProfileName(profiles, browserType, profileName) {
  const appendWithinLimit = (suffix) => {
    const characters = Array.from(profileName);
    while (`${characters.join('')}${suffix}`.length > MAX_PROFILE_NAME_LENGTH) {
      characters.pop();
    }
    return `${characters.join('')}${suffix}`;
  };
  const baseName = appendWithinLimit(' 副本');
  if (!isDuplicateProfileName(profiles, browserType, baseName)) return baseName;

  let suffix = 2;
  let candidate = appendWithinLimit(` 副本 (${suffix})`);
  while (isDuplicateProfileName(profiles, browserType, candidate)) {
    suffix += 1;
    candidate = appendWithinLimit(` 副本 (${suffix})`);
  }
  return candidate;
}

function createProfileExport(profiles) {
  return {
    version: 1,
    profiles: profiles.map(({ browserType, name }) => ({ browserType, name })),
  };
}

function validateProfileImportDocument(document) {
  if (
    !document
    || typeof document !== 'object'
    || document.version !== 1
    || !Array.isArray(document.profiles)
    || document.profiles.length > 1000
  ) {
    throw new Error('Invalid profile metadata document');
  }

  const profiles = document.profiles.map((profile) => {
    validateProfileInput(profile?.browserType, profile?.name);
    return { browserType: profile.browserType, name: profile.name };
  });
  for (let index = 0; index < profiles.length; index += 1) {
    if (isDuplicateProfileName(
      profiles.slice(0, index),
      profiles[index].browserType,
      profiles[index].name,
    )) {
      throw new Error('Duplicate profile metadata');
    }
  }
  return profiles;
}
...
function filterRestorableProcessRecords(baseDir, profiles, records) {
  if (!Array.isArray(profiles) || !Array.isArray(records)) {
    return [];
  }

  const safeProfiles = new Map(
    profiles
      .filter((profile) => isStoredProfilePathSafe(baseDir, profile))
  ...
  return records.filter((record) => {
    ...
    const profile = safeProfiles.get(record.profileId);
    return Boolean(
      profile
      && record.browserType === profile.browserType
      && record.profilePath === profile.path,
    );
  });
}
