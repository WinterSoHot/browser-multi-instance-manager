...
function registerIpcHandlers({
  ...
  function sanitizeAppSettings(settings) {
    ...
    return { closeToTray: settings.closeToTray };
  }
  ...
  function sanitizeResult(channel, result, args) {
    ...
    if (channel === 'get-app-settings') return sanitizeAppSettings(result);
    if (channel === 'set-app-settings') {
      if (result?.success === true) {
        return { success: true, settings: sanitizeAppSettings(result.settings) };
      }
  ...
  function fallbackResult(channel, args) {
    ...
    if (channel === 'get-app-settings') return { closeToTray: true };
