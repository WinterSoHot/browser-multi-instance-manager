...
function validateUpdateCheckCacheResult(result) {
  if (!isPlainRecord(result)) throw new Error('Invalid update check cache');
  const status = ownDataValue(result, 'status');
  if (status === 'current' && hasOnlyKeys(result, ['status'])) {
    return { status: 'current' };
  }
  if (status !== 'available' || !hasOnlyKeys(result, ['releaseUrl', 'status', 'version'])) {
    throw new Error('Invalid update check cache');
  }

  const version = ownDataValue(result, 'version');
  const releaseUrl = ownDataValue(result, 'releaseUrl');
...
function validateUpdateCheckCache(cache) {
  ...
  if (!isPlainRecord(cache) || !hasOnlyKeys(cache, ['checkedAt', 'checkedVersion', 'result'])) {
    throw new Error('Invalid update check cache');
  }

  const checkedAt = ownDataValue(cache, 'checkedAt');
  const checkedVersion = ownDataValue(cache, 'checkedVersion');
  const result = ownDataValue(cache, 'result');
...
function isRawUpdateCheckCache(cache) {
  try {
    validateUpdateCheckCache(cache);
    return true;
  } catch {
    return false;
  }
...
function validateRawStoreData(data) {
  ...
  if (
    ...
    || (hasOwn(data, 'updateCheckCache') && !isRawUpdateCheckCache(data.updateCheckCache))
  ) {
    throw new Error('Invalid app store data');
  }
...
function migrateStoreData(input = {}) {
  ...
  const migrated = {
    ...
    updateCheckCache: hasOwn(input, 'updateCheckCache')
      ? validateUpdateCheckCache(input.updateCheckCache)
      : null,
  };
...
function createAppStore(store) {
  ...
  return {
    ...
    setUpdateCheckCache: (cache) => set('updateCheckCache', validateUpdateCheckCache(cache)),
  };
}
