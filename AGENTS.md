...
Use CommonJS JavaScript, HTML, and CSS. Follow surrounding two-space, semicolon, trailing-comma, `camelCase`, quote, and CSS-variable conventions. Expose only required renderer methods through `preload.js`.

Add named `test/*.test.js` regressions and run `npm test`. Smoke-test affected profile actions, filters, views, and browser paths with `npm start`; verify platform changes on that OS. PRs need a behavior/platform summary, linked issues, relevant screenshots, and manual checks. Use focused Chinese commit subjects: `新增`, `修复`, `优化`, or `更新`.
...
Diagnostics expose stable, sanitized states. Recreate only after a fresh explicit `stopped` result and exact expected-profile-path match; never repair running/unknown profiles. Unknown recovery permits only retry or warned tracking clear; never signal a PID without fresh exact executable/profile-path verification.
...
Profile removal preserves data unless explicitly moved with Electron’s trash API. “Forget process” only clears tracking after warning acknowledgement. Tree termination is limited to app-launched processes. Main-process launch, delete, rename, and diagnostics repair share per-profile lifecycle coordination.
